...
int main(int argc, char* argv[])
{
	...
	GLFWwindow* window = glfwCreateWindow(width, height, "Exercise 4", nullptr, nullptr);
	if (window == NULL)
	{
		glfwTerminate();
	...
	//1. Make a fps function (put in wherever you want)
	//the idea is to compute how much time has passed since the last frame
	//then use that to find how many frame are rendered in one second
	//use glfwGetTime to know the present time
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//"1. Call your fps function at each loop of animation
